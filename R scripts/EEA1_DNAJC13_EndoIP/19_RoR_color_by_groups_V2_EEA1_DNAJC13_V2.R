...
main_data_file  <- "ea20112ea20113all_stats_results_exp1analysis1.csv"
color_data_file <- "19_color_by_groups_refined.csv" 

col_x     <- "log2FC_EEA1_rescue.EEA1KO"
col_y     <- "log2FC_DNAJC13rescue.DNAJC13KO"
...
# --- NEW: Extract unique terms in the EXACT order they appear in the CSV ---
# This ensures the legend and color mapping follow the file order.
ordered_groups <- unique(term_data$term)
ordered_groups <- ordered_groups[ordered_groups != "Other"] 
...
# --- FIX: Set factor levels based on CSV order, putting "Other" at the end ---
data$term <- factor(data$term, levels = c(ordered_groups, "Other"))
...
# Base colors list
# Group 1 (from CSV) -> #c0392b, Group 2 -> black, etc.
base_colors <- c("#c0392b", "black", "#085990","#27ae60","#e67e22" , "#8e44ad")

if(num_groups > length(base_colors)) {
  # If CSV has more groups than base_colors, generate a palette for the remainder
  extra_count <- num_groups - length(base_colors)
  extra_colors <- colorRampPalette(brewer.pal(8, "Set3"))(extra_count)
  final_palette <- c(base_colors, extra_colors)
} else {
  final_palette <- base_colors[1:num_groups]
}

# Explicitly map names to colors to ensure 1-to-1 consistency
names(final_palette) <- all_levels

# Force "Other" to be grey regardless of its position in the list
if("Other" %in% names(final_palette)) {
  final_palette["Other"] <- "grey75" 
}

# Sort data so "Other" (grey points) are plotted first (in the background)
...
# 修改这一行，让具有不同坐标的同名基因也能拿到标签
label_data <- data %>% 
  filter(term != "Other") %>% 
  distinct(Gene.Symbol, x, y, .keep_all = TRUE)
#########################################################################
# 3. 统计信息 (Statistics)
#########################################################################
...
p1 <- draw_axis_line_with_edges(6, 6) + 
  ...
  annotate("text", x = 5.8, y = -1.5, label = equation_parsed, parse = TRUE, hjust = 1, size = 4.5) +
  annotate("text", x = 5.8, y = -2.0, label = stats_text, parse = TRUE, hjust = 1, size = 4.5)

print(p1)

# Save files
ggsave("19_RoR_color_by_groups_V2_EEA1_DNAJC13_V2.png", plot = p1, height = 8, width = 12, dpi = 300)
ggsave("19_RoR_color_by_groups_V2_EEA1_DNAJC13_V2.pdf", plot = p1, height = 8, width = 11, device = "pdf", useDingbats = FALSE)